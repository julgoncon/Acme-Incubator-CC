...
@Service
public class EntrepreneurDemandCreateService implements AbstractCreateService<Entrepreneur, Demand> {
	...
	public void unbind(final Request<Demand> request, final Demand entity, final Model model) {
		...
		Integer investmentId = entity.getInvestmentRound().getId();
		model.setAttribute("investmentId", investmentId);

	}

	@Override
	public Demand instantiate(final Request<Demand> request) {
		assert request != null;

		Demand result = new Demand();
		Integer investmentId = request.getModel().getInteger("investmentId");
		InvestmentRound investmentRound = this.repository.findInvestmentRoundById(investmentId);
	...
	@Override
	public void create(final Request<Demand> request, final Demand entity) {
		assert request != null;
		assert entity != null;
